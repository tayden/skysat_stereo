...
def getparser():
    parser = argparse.ArgumentParser(description='Visualize stereo disparity, intersection error and DEM for one or more stereo run directories')
    parser.add_argument('dirs',nargs='+',help='one or more stereo run directories containing *-F.tif, *-L.tif, *-R.tif, *In*.tif and *-DEM.tif')
    return parser

def main():
    parser = getparser()
    args = parser.parse_args()
    dir_list = [os.path.abspath(x) for x in args.dirs]
    for dir in dir_list:
